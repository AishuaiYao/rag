...
class HybridRetriever:
    def __init__(self, file_name, chunks):
        embedding = HuggingFaceEmbeddings(model_name="models/BAAI/bge-large-zh-v1.5", model_kwargs={"device": "cpu"})
...
class EnhancedRAG:
    ...
    def generate_prompt(self, question, contexts):
        ...
        # 创建提示模板，要求基于上下文回答问题
        return f">你是一个case分析助手，请严格根据以下来源的case里给出的best和reason情况，判断问题里哪个path更好，并且答案的输出格式与来源的case里的形式一样添加best和reason\n  {context_str} \n>按步骤思考并回答：{question}\n>如果上下文信息不足，请明确指出缺失的信息。最后用中文给出结构化答案。"
